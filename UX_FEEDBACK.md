...
- [ ] (minor) [operator] The vendor arm shows `d.deadScripts[0]` only (panel.html:1592) while the array holds both dead chunks, with nothing saying a second exists. Evidence presented as complete when it is one of two undercuts the reason the line is there at all. Fix: show the count ("2 files, first: …") or list both.
- [ ] (minor) [operator] The explanatory paragraph uses `.why`, which is 11.5px `--muted` (panel.html:371), against the 15px `--ink` body type it has just replaced. The three sentences carrying the entire meaning of the surface are set smaller and dimmer than the book text the reader was in. Fix: give `#reader-failed-why` 13px and `--ink` on this surface.
- [ ] (minor) [operator] `.failtitle` is a `<p>` (panel.html:696), so a screen-reader user cannot reach the failure by heading navigation and the surface has no heading anywhere; the panel already sets headings in the same face via `h2`. Fix: make it an `h2` or add `role="heading" aria-level="2"`.
- [ ] (minor) [operator] `button.act[disabled] { opacity: .45 }` (panel.html:416) composites `--ink` #E4E8EE over `--ground` #0E1116 to roughly #6E7277, which measures 3.91:1 against the ground. Inside the `⋯` menu, where `reader-pager` is displaced and is visible, the operator has to read "Next ›" and "See the real page" at under 4.5:1 to learn which controls are unavailable. Fix: dim with a token colour on the label instead of an opacity on the whole button.
- [ ] (minor) [operator] `say(why.slice(0, 80))` (panel.html:2399) cuts the daemon's sentence at exactly 80 characters with no ellipsis, so the footer line ends mid-word and reads as a rendering fault rather than as a truncation. Fix: trim to a word boundary and append "…".
- [ ] (minor) [operator] `#reader-failed-why` carries no `overflow-wrap`, unlike `.failwhat` which sets `anywhere` (panel.html:499). When panel.html:2395 writes a raw daemon reason containing a long path or URL into it, the unbroken token forces the overlay's computed `overflow-x: auto` into a horizontal scrollbar inside a 640px panel. Fix: add `overflow-wrap: anywhere` to `.why` on this surface.
