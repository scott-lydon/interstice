...
- [ ] (minor) [operator] The vendor arm shows `d.deadScripts[0]` only (panel.html:1592) while the array holds both dead chunks, with nothing saying a second exists. Evidence presented as complete when it is one of two undercuts the reason the line is there at all. Fix: show the count ("2 files, first: …") or list both. {worthwhile}
...
- [ ] (minor) [operator] `.failtitle` is a `<p>` (panel.html:696), so a screen-reader user cannot reach the failure by heading navigation and the surface has no heading anywhere; the panel already sets headings in the same face via `h2`. Fix: make it an `h2` or add `role="heading" aria-level="2"`. {worthwhile}
...
- [ ] (minor) [operator] `say(why.slice(0, 80))` (panel.html:2399) cuts the daemon's sentence at exactly 80 characters with no ellipsis, so the footer line ends mid-word and reads as a rendering fault rather than as a truncation. Fix: trim to a word boundary and append "…". {worthwhile}
