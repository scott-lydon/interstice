...
async function flashcards(config, ctx) {
  ...
  return {
    ...
    detail: {
      deck: queue.deck,
      // Not a constant: `leastStudiedQueue` returns 'configured_deck' when a specific deck is set
      // and it is not the least studied one, and the delivery record has to agree with the panel.
      chosenBy: queue.reason,
      meanReps: queue.metric.meanReps,
      due: queue.metric.due,
