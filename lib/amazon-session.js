 ...
 *
 * The reader runs its own browser profile, kept apart from the one you browse in, and the price
 * of that isolation is that Amazon does not know you there: the reading rung lands on a sign-in
 * page instead of on your book.
 *
...
// Amazon spreads a signed-in session over several hosts, so the filter has to keep
// `read.amazon.com`, `www.amazon.com` and `.amazon.com` together while leaving every other site
// in your browser where it is.
//
// Anchored deliberately. `LIKE '%amazon.com'` reads like a domain match and is a suffix match:
// it also selects `notamazon.com` and `fakeamazon.com`, so a lookalike's cookies were copied
// into the reader profile and then spared by the prune below, which deletes everything this
// same clause does not match. The exact-or-dot-prefixed form is the domain match the sentence
// above always claimed.
const AMAZON = "(host_key = 'amazon.com' OR host_key LIKE '%.amazon.com')";
 ...
 * asked of it. The value stays sealed: this is the ciphertext itself, hex-encoded, so nothing
 * here has to decrypt anything in order to compare two of them.
 */
