...
export async function findBinauralTracks(config, { timeoutMs = 8000 } = {}) {
  const binaural = config.companions?.binaural ?? {};
  const app = binaural.app ?? 'Music';
  const terms = (binaural.search ?? ['binaural', 'isochronic', 'solfeggio', 'hz']).map(safeTerm).filter(Boolean);
 ...
 * minutes, which is the wrong answer for a button that offers a whole interval.
 */
