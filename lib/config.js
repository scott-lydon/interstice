 ...
 *
 * Cached and frozen after the first call, because every rung and every route reads it and a config
 * that changed mid-decision would make the logs unreadable. Pass `force` to re-read, which is what
 * `writeUserConfig` arranges by clearing it.
 *
 * Named `loadCached` rather than `load` because that memo is the whole surprise of this function:
 * it was called `load`, which reads as "go and read the file", while the second call after an edit
 * to the file returned the first call's answer and nothing in the name said so. A caller now has
 * to type the word `Cached` to get the cached thing, and `defaults()` above is right there for the
 * caller who genuinely wants the file as it is on disk.
 *
 ...
 */
export function loadCached({ force = false } = {}) {
...
/**
 * Merges a patch into the user's config file, refusing to write an invalid result.
 *
