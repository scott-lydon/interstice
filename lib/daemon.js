...
export class Daemon {
  constructor({ config, logger = createLogger({ toFile: true }) }) {
    ...
    // to name 33 members of the Reader directly, three of them plain fields, which meant an HTTP
    // route could only be understood by reading a 1747-line class as well. See lib/reading.js.
  ...
  #attach(source, name) {
    source.on('submit', (ev) => {
      ...
      // identically whichever one drove it (5.5).
