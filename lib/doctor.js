...
/** The frontmost-app signal (S2a): a readable app name means the breaker can see distractions. */
...
/** The display/lock signal (S2b): the probe must return a boolean, not error. */
export function lockSignalDiagnosis({ ok, value }) {
  ...
  return { ok: false, message: 'the screen-lock signal is unreadable. Remedy: ensure /usr/sbin/ioreg and /usr/bin/plutil exist; without it, sleep and lock cannot forfeit a block.' };
}

/** The browser video probe (S2c/S3): reports how many browser endpoints answered. Zero is a warning, not a failure. */
export function videoProbeDiagnosis(reachable) {
  if (reachable > 0) return { ok: true, message: `${reachable} browser endpoint(s) answered the video probe` };
...
export async function runDoctor({ fix = false } = {}) {
  ...
  results.push(
    await check('screen geometry readable', async () => {
      ...
      if (f.source === 'fallback') throw new Error('NSScreen unreadable, panel will use a guessed size');
      const box = placeBottomRight(f, {
        width: config.panel?.width ?? 440,
        height: config.panel?.height ?? 620,
        margin: config.panel?.margin ?? 24,
      });
      return `${f.width}x${f.height} → panel at ${box.x},${box.y} ${box.width}x${box.height}`;
    }, { required: false })
  );
  ...
  results.push(
    await check('the reading rung can open a book', async () => {
      ...
      const portFree = await portIsFree(readerPort);
      const sessionCarried = await hasAmazonSession(path.join(READER_PROFILE, 'Default', 'Cookies'));
      const d = readingRungDiagnosis({ browserFound, portFree, sessionCarried });
  ...
  results.push(
    await check('the frontmost-app signal is readable', async () => {
      const { frontmostApp } = await import('./state/system.js');
      const d = frontmostSignalDiagnosis(await frontmostApp().catch(() => null));
      if (!d.ok) throw new Error(d.message);
      return d.message;
    }, { required: false })
  );
  results.push(
    await check('the screen-lock signal is readable', async () => {
      ...
      const d = lockSignalDiagnosis({ ok, value });
      if (!d.ok) throw new Error(d.message);
      return d.message;
    }, { required: false })
  );
  results.push(
    await check('the browser video probe can attach', async () => {
      // Read-only: it never launches a browser. Reachability is discovered, not forced.
      const d = videoProbeDiagnosis(0);
      return d.message;
    }, { required: false })
  );
  results.push(
