...
/**
 * The browser video probe endpoint: how many of the configured endpoints answered.
 *
 * Named for what it does. This asks `/json/version` and nothing more, so it reports that an
 * endpoint is THERE, not that the probe can read a play state through it. Those came apart once
 * already: `probeVideo` was skipping `Target.attachToTarget`, every tab reported not-playing
 * forever, and `/json/version` answered perfectly throughout. A check called "can attach" would
 * have been green for that whole bug. Attaching for real is `test/video-breaker.pw.mjs`, which
 * drives a real browser playing a real file.
 *
...
export function videoProbeDiagnosis(reachable, configured = null) {
  if (reachable > 0) {
    return { ok: true, message: `${reachable} of ${configured ?? reachable} browser endpoint(s) answered on /json/version` };
  }
  if (configured === 0) {
    return {
      ok: true,
...
export async function runDoctor({ fix = false } = {}) {
  ...
  results.push(
    await check('Claude Code hooks registered', async () => {
      ...
      // Per event, not one substring match over the whole object. A settings file carrying only
      // one of the two used to print both, and a stray "interstice" inside any unrelated event
      // satisfied it, so this check could pass on a half-installed hook set.
      const registered = Object.entries(s.hooks ?? {})
        .filter(([, entries]) => JSON.stringify(entries).includes('interstice'))
        .map(([event]) => event);
      const wanted = ['UserPromptSubmit', 'Stop'];
      const missing = wanted.filter((e) => !registered.includes(e));
      if (missing.length) {
        throw new Error(
          `${missing.join(' and ')} not installed (found: ${registered.join(', ') || 'none'}). `
          + 'Remedy: run `interstice install`, which writes both and leaves your other hooks alone.'
        );
      }
  ...
  results.push(
    await check('the library is readable at all', async () => {
      ...
      // `started` is the row count of a LIMIT 40 query, so this is a floor, not a total.
      return `at least ${state.started} book${state.started === 1 ? '' : 's'} with a saved position`;
    }, { required: false })
  );
  results.push(
  ...
  results.push(
    await check("the reading rung's preconditions hold", async () => {
  ...
  results.push(
    await check('the screen-lock signal is readable', async () => {
      const { screenLockedStrict } = await import('./focus/breakers/display.js');
      let ok = true, value;
      try { value = await screenLockedStrict(); } catch { ok = false; }
  ...
  results.push(
    await check('the browser video probe endpoint answers', async () => {
