 ...
 *
 * Timers, not polling: arming schedules one callback per threshold, plus a short retry chain
 * after an idle veto (`#scheduleRetry`, up to `vetoRetries`). Nothing else runs while a gap is
 * open, and nothing runs at all while one is not.
 *
...
export class Engine extends EventEmitter {
   ...
   *
   * `toISOString` is UTC, so keying a day off it expired a day-long stand down at 17:00 local
   * west of Greenwich rather than at midnight: you asked to be left alone for the day and the
   * router came back that evening. `localISO` carries the machine's own offset, which is the
   * same reason the focus tracker uses it to decide which day a star belongs to.
   */
  #dayKey(ts) {
    return localISO(new Date(ts)).slice(0, 10);
  }

  /** A day-long stand down expires when the local calendar date changes. */
  #standDownActive() {
  ...
  async #deliver(gap, rung, reason) {
    try {
      const result = await this.doDeliver(rung, this.config, { panel: this.panel });
  ...
  /**
   * Take back a day-long stand down.
   *
   * `standDown({ forDay: true })` was one click on the dashboard with no confirmation,
   * and nothing in the UI or the API could clear it: the only thing that ever reset
   * `stoodDownForDay` was the calendar date changing. An irreversible control with no
   * undo is a trap, so this is the undo.
   */
  resumeDay() {
    const wasStoodDown = this.stoodDownForDay;
    this.stoodDownForDay = false;
    this.standDownDate = null;
    this.#log({ kind: 'stand_down_cleared', gapId: this.gap?.id ?? null, wasStoodDown });
    return { ok: true, wasStoodDown };
  }
