...
//
// Interstice's own panel is permanently exempt, by process id rather than by name, so a user
// cannot break it by editing config. The constant below covers the hotkey applets, which do run
// under their own name. The blacklist itself is opt-in via `focus.blacklistApps`.
