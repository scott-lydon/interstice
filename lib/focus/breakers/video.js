// Breaker: non-whitelisted online video playing forfeits the focus block. Browsers only,
...
export function decideVideo(records, { whitelist = [] } = {}) {
  for (const r of records) {
    ...
    if (r.readerProfile) continue; // the reader's own profile never breaks its own block
    let allowed;
    try {
