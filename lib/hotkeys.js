...
function scriptFor(app, port) {
  ...
  // The header is double-quoted inside the shell command, and everything else stays single
  // quoted. `sh` does not substitute inside single quotes, so the first version of this shipped
  // the six characters `$(cat ` to curl as a literal header value: every hotkey press was a 401
  // that `|| true` then swallowed, and the apps looked like they were doing nothing at all.
  // `\\"` is a double quote inside the AppleScript string, which is itself inside a template.
  return [
