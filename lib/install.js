...
import { loadCached as loadConfig } from './config.js';
 ...
 * and only if it is genuinely a node 22 or newer.
 */
export async function resolveNode({ execPath = process.execPath } = {}) {
  ...
  for (const candidate of stable) {
    ...
    try {
      ...
      if (major >= 22) return { path: candidate, stable: true, version: stdout.trim() };
    } catch {
...
/** Pull the pid and LastExitStatus out of launchd's old-style plist output. */
...
export async function install({ force = false } = {}) {
  ...
  // The second half of what `reloadLaunchAgent` promises: prove the job launchd just started is
  // the one answering the
