 ...
 * network. It measures 0.6s a page at 1.00 confidence on this book.
 *
 ...
 * paragraphs.
 *
 * **Tall lines are a heading.** Measured against the page's own median, because the
 * type size is your setting and a fixed threshold would call every line a heading
 * at one size and none at another.
 *
...
export function toBlocks(lines) {
  ...
  const isHeading = (l) => l.height > typical * 1.35;
  ...
  const gapsOfKind = (heading) =>
    body
      .slice(0, -1)
      .map((l, i) => (isHeading(l) === heading && isHeading(body[i + 1]) === heading ? gapAfter(i) : null))
  ...
  for (let i = 0; i < body.length; i += 1) {
    const line = body[i];
    const thisKind = isHeading(line) ? 'heading' : 'para';
