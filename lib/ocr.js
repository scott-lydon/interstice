 ...
 * sits would silently eat a line of the book. `isChrome` below is where the two meet;
 * this constant is only the content half.
 */

/** Text the reader draws around the book: toolbar labels, the page and location lines. */
export const CHROME_PATTERNS = [
  ...
  // toolbar reads as one line once its pieces are joined, so this is anchored to the whole
  // joined line rather than to any one glyph.
 ...
 *
 * Four signals, none of which works alone on a justified page.
 *
 ...
 * paragraphs.
 *
 * **A gap wider than the usual line spacing ends a block.** Measured per kind, because a
 * heading's lines sit further apart than a paragraph's, so one threshold for the page would
 * split every heading or join every paragraph.
 *
...
export function toBlocks(lines) {
  ...
  // of it. A chapter opening whose large type outnumbers the prose on the page pulls the median up to
  // heading size, so measured against itself nothing is a heading and the title joins the first
  // paragraph.
  ...
  const typicalGapFor = {
    heading: median(gapsOfKind(/* heading= */ true)),
    para: median(gapsOfKind(/* heading= */ false)),
  };
  const blocks = [];
  let current = [];
  let kind = null;
...
export async function pageText(jpeg, opts) {
  ...
  return {
    ...
    lines: lines.length,
    // The denominator `weak` is a fraction OF. `lines` counts the reader's own furniture too, and
    // multiplying `weak` by that rescaled a score onto a population that was never scored: five
    // weak of eleven body lines on a fifteen-line page rendered as "7 of 15".
    scoredLines: confidences.length,
