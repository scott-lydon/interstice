...
/**
 * The window. One of them, bottom right, and it is where every rung arrives.
 *
 ...
 * interfaces and are never brought to the front by the router. Two things you press are
 * exceptions: signing in to Amazon, and "Open the Kindle app instead". The
 ...
 *
 * It is a Chromium app window rather than a native one because the project has no build step and no runtime dependencies, and `--app` gives a chromeless window that behaves
...
export class Panel {
   ...
   * no error anywhere to say why. Seen here once: a pid that had been alive for days,
  ...
  stop() {
    ...
    // they may be reading, and nothing in this project closes a window you opened.
    this.child = null;
  }
}
