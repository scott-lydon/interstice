...
export async function screenFrame({ timeoutMs = 3000 } = {}) {
  try {
    ...
    const dock = f.visibleY;
    const menuBar = f.height - f.visibleHeight - dock;
    return {
      width: f.width,
      height: f.height,
      top: menuBar,
      bottom: f.height - dock,
      left: f.visibleX,
      right: f.visibleX + f.visibleWidth,
      source: 'NSScreen',
    };
  } catch {
...
export function placeBottomRight(frame, { width, height, margin }) {
  const w = Math.min(width, frame.right - frame.left - margin * 2);
  const h = Math.min(height, frame.bottom - frame.top - margin * 2);
  return {
    width: Math.round(w),
    height: Math.round(h),
    x: Math.round(frame.right - w - margin),
    y: Math.round(frame.bottom - h - margin),
  };
}
...
export class Panel {
  ...
  async #spawn() {
    ...
    const box = placeBottomRight(frame, s);
