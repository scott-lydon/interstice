...
export const PROBE = `(() => {
  ...
  return JSON.stringify({
    ...
    // Amazon's own loading element, by its own class name, measured off a wedged reader
    // rather than guessed at: a page showing nothing but this reported one svg and a body
    // fourteen characters long. Reported separately from painted so a caller can tell
    // "still loading" from "loaded something that is not the book", which are different
    // failures with different remedies. No backticks in this comment: it lives inside a
    // template literal, which is the warning written twenty lines below and the one I just
    // walked into.
...
export class Reader {
  ...
  async revive({ clearFirst = false } = {}) {
    ...
    try {
      ...
      this.error = null;
      // A sign-in page counts as arrived: the reader is not wedged, it is asking for something,
      // and the panel has a surface for that. Amazon's failure page counts for the same reason.
      // Nothing arriving is the one case this is here to stop reporting as a recovery.
  ...
  async state() {
    ...
    return {
      ...
      //
      // Gated on the browser having a label of its own, which it does whenever it is
      // showing a page at all. Without that gate the fallback fires in the one case it
      // was never meant for: a browser showing NOTHING has no label, so the shelf's
      // remembered one was served instead, and a blank reader reported the page you
      // were on before it went blank. Measured during a cold start: the probe's label
      // was empty for forty seconds while this line kept answering `Page 217 of 220`.
