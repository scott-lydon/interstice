...
import { connect, findBrowser, launchBrowser, requireWebSocket } from './cdp.js';
 ...
 * from the file, and Chrome 137 onwards ignores `--load-extension` so the page
 ...
 *
 * The page label is the only place the reader states its position in words, and it
...
export const PROBE = `(() => {
  ...
  return JSON.stringify({
    ...
    // How many device pixels the page already believes it has to a point. The clip
    // scale is derived from this rather than fixed: the two multiply, so a constant
    // 2 against a page already at 2 gives a four-times-oversized picture.
 ...
 * capture is already clipped to the page rectangle (see `CLIP`), so the toolbar and
 ...
 * stays fast regardless: 65ms measured while the renderer was at 96% of a core. At a
 * quarter scale it is a few kilobytes, which is nothing to encode and nothing to
 ...
 * time down the page. Nothing else in its display settings is touched. The font,
 * the theme and the margins are yours, they sync from your Kindle, and a tool that
 * quietly re-set them would be worse than one that renders badly.
 */
...
export class Reader {
   ...
   * is the path that does not depend on one, and it is also the only path a passkey
   * can take: Chrome draws that QR code in its own window chrome, so a browser with
   ...
   * two can only be loaded with `--load-extension`, which Chrome has ignored since
   * 137. So the sign-in that a person actually has to type goes where typing it is
   * one Touch ID press.
   *
   ...
   *
   * Which is why this is not offered on its own. `reauthenticate` runs first and
  ...
  async #runSignIn() {
    ...
    const bin = findBrowser(fs);
    if (!bin) throw new Error('no Chromium-family browser found to sign in with. Remedy: install Google Chrome (or another Chromium-family browser) so the reader profile has one to drive.');
   ...
   *
   * Only when there is nothing there. A profile that has been signed in is left
   * alone: its cookies are newer than the ones in your ordinary browser, and
  ...
  async reauthenticate() {
    ...
    if (carried.carried === 0) return { ...carried, signedIn: false };
  ...
  async #start() {
    requireWebSocket();
    const bin = findBrowser(fs);
    if (!bin) throw new Error('no Chromium-family browser found to render the book. Remedy: install Google Chrome (or another Chromium-family browser) so the reader has one to render into.');
  ...
  async revive({ clearFirst = false } = {}) {
    ...
    try {
      ...
      await this.settle();
      await this.#evaluate(CSS_INJECT).catch(() => {});
      await this.#seed();
   ...
   */
  async retryBook() {
    if (!this.running) return { ok: false, reason: 'the reader is not open' };
    const reopened = await this.revive({ clearFirst: true });
    this.logger?.info?.('reader: retried the book', { cleared: this.cleared, reopened });
    return { ok: reopened, cleared: this.cleared, reopened };
  }

  /**
   * Size the window, then believe it.
   *
   * Chrome clamps: ask for 412 wide and you get 480, which is the narrowest page it
   * will lay out. What comes back is read off the page itself rather than assumed,
   * because that number is also what a click in the panel is scaled against, and a
   * click mapped through a size the page never had lands somewhere else.
   */
  /**
  ...
  async settle({ timeoutMs, graceMs = 6000 } = {}) {
    const deadline = Date.now() + (timeoutMs ?? this.settings.loadTimeoutMs);
    let last = null;
    let paintedAt = 0;
    while (Date.now() < deadline) {
      await new Promise((r) => setTimeout(r, 400));
  ...
  async dismissOverlays(answer = 'No') {
    try {
      ...
      const answered = JSON.parse((await this.#evaluate(dismissScript(answer), { timeoutMs: 2500 })) || '[]');
      if (answered.length) {
   ...
   *
   * `scale: 2` is also the only way to get device pixels out of this browser.
   * `deviceScaleFactor: 2` in the metrics override changes the page's own idea of
   * its density and does nothing at all to the screenshot: measured, an unclipped
   * shot of a 480x400 viewport came back 480x400, while the same page clipped to
   * 384x253 at scale 2 came back 768x506. The panel has been showing a 1x book on a
   * Retina display this whole time.
   */
  /**
   * How much to multiply the clip by, so a page arrives at two device pixels to the
   * point however the browser is behaving today.
   *
   * `clip.scale` and the page's own `devicePixelRatio` multiply, and which of them
   * is in force is not something to assume: an unclipped shot of this reader came
   * back at exactly its CSS size, as if the density override did not exist, while a
   * clip at scale 2 on the same reader after a restart came back at four times. A
   * fixed 2 is therefore right half the time. Measured against the page instead.
   */
  captureScale() {
    return 1;
  }
  ...
  async #shoot({ scale = null, quality = null, timeoutMs = 12000 } = {}) {
    if (!this.clip) await this.#probe().catch(() => {});
   ...
   * screenshots nor navigations. One scale, always. A capture is 65ms.
   */
  #watch() {
   ...
   */
  async capture({ force = false, minIntervalMs = 350 } = {}) {
    this.lastUsedAt = Date.now();
   ...
   * number that is not its own, and stops rather than making you queue behind three
   * speculative turns.
   */
  /**
