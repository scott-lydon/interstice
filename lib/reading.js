...
//
// UC-MOD-001. lib/server.js used to reach through the daemon into the Reader 33 times, and not
// only at methods: `daemon.reader.seq`, `daemon.reader.running` and `daemon.reader.signingIn` are
// plain fields of a 1747-line class. The cost was not stylistic. Fixing anything about the reading
...
// larger change (UC-CUPID-002), and the point of this one is that when that split happens, only
 ...
 * this is only ever reached when the input did not change anything on screen (a click on dead
 * space, a key the reader ignored).
 */
...
export function createReadingSurface({ reader }) {
  return {
    ...
    async view({ asin, width, height }) {
      ...
      // Keep a page or two ahead of you read. Not awaited: this poll must answer at the speed of a
