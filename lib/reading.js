...
export function createReadingSurface({ reader }) {
  return {
    ...
    async view({ asin, width, height }) {
      ...
      //
      // The state just read is handed on, so capture can refuse to photograph a page that is not
      // showing the book without paying for a second probe of its own.
    ...
    /** The current picture of the page, pulled on demand, or null when it has nothing on screen yet. */
    frame() {
