...
import { readJsonl, readJsonlWithErrors, appendJsonl } from './logger.js';
...
import { UnknownInputError } from './reading.js';
...
/**
 * Thrown when a request body arrives that is not the JSON it claims to be.
 *
 * It exists so the dispatcher can answer 400 rather than 500: a body this daemon cannot parse is
 * the caller's mistake, and a status code that says so is the difference between "you sent me
 * something broken" and "I am broken".
 */
class BadRequestBodyError extends Error {
  constructor({ error, bytes, position }) {
    super(
      `request body is not valid JSON: ${error}` +
        (position === null ? '' : ` (at byte ${position} of ${bytes})`) +
        `. Remedy: send a JSON object, or send no body at all for an empty one.`
    );
 ...
 *
 * A malformed body used to resolve to `{}`, which turned every parse failure into whatever the
 * handler says about a missing field: POST /api/queued answers "empty" to a body that
 * did contain text and was merely truncated in transit, and the one true fact, that the JSON did
 * not parse, was nowhere in the answer. Now it throws and the dispatcher turns it into a 400 that
 * names the parse error.
 *
 * An absent body is still `{}`: sending nothing is a legitimate way to say "no arguments", and the
 * routes that need a field already reject a missing one on their own terms.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        // V8 puts the offending offset in the message for most syntax errors and omits it for
        // "Unexpected end of JSON input". Reported when it is there, null when it is not, rather
        // than a fabricated zero: a wrong offset is worse than no offset.
        const at = /position (\d+)/.exec(err.message);
        reject(
          new BadRequestBodyError({
            error: err.message,
            bytes: Buffer.byteLength(data),
            position: at ? Number(at[1]) : null,
          })
        );
      }
    });
 ...
 * makes every iteration cost twelve minutes. A gap opened from `/debug/submit` is tagged `synthetic: true` and excluded from the
 * statistics. The other debug routes act on whatever gap is open, real or not, so they can
 * change a real one; that is the point of them, and it is why only the opener tags.
 */
...
export async function createServer({ daemon, config }) {
  ...
  const routes = {
    ...
    // `malformed` rather than a silent skip. Every figure below is computed from this one file, so
    // a line that will not parse is a number that is quietly wrong; the dashboard shows the count
    // and the first line numbers so a wrong figure can be told from a slow day.
    'GET /api/gaps': (req, res, url) => {
      const limit = Number(url.searchParams.get('limit') || 500);
      const { records: all, malformed } = readJsonlWithErrors(GAPS_LOG);
      json(res, 200, { count: all.length, gaps: all.slice(-limit), malformed });
    },

    'GET /api/stats': (req, res) => {
      const { records: gaps, malformed } = readJsonlWithErrors(GAPS_LOG);
      json(res, 200, { ...summarize(gaps, config), malformed });
    },
    ...
    'POST /api/panel/ping': (req, res) => {
      daemon.panel.ping();
      const panelState = daemon.panel.state();
    ...
    'GET /api/focus': (req, res) => {
      // Local offset, not Z, for `at` and the block times: their date part is read back by
      // slicing, so a UTC stamp files an evening block on tomorrow. The latency timestamps
      // beside them are ISO from the engine's own epoch millis and are never sliced.
      const at = localISO();
      json(res, 200, {
        ok: true,
        at,
    ...
    /** Your configured deck, or the least studied one when none is set, with its cards. */
    ...
    'GET /api/reading': async (req, res) => {
      ...
      // allow it, and `POST /api/reading/view` calls this same snapshot on every 1.5s poll:
      // uncached, one open panel is a permission prompt every few seconds.
    ...
    'POST /api/reading/view': async (req, res) => {
      // Started together. Draining the request body waits on the panel's socket and the snapshot
      // waits on the Kindle store, and neither is an input to the other, so waiting for them one
      // after the other spends the sum of two unrelated latencies on a route that runs every poll.
      const [body, state] = await Promise.all([readBody(req), readingSnapshot(config)]);
      ...
      try {
        // The reader's own sequencing (open, recover a lost session, capture, read ahead) is the
        // reading surface's, not this route's. What is left here is what is genuinely HTTP: the
        // asin this request is about, and the book's name and position, which come from the Kindle
        // store rather than from the browser.
        const view = await daemon.reading.view({
          asin,
    ...
    'GET /api/reading/text': async (req, res, url) => {
      if (!daemon.reading.status().running) return json(res, 409, { ok: false, error: 'the reader is not open' });
      try {
        ...
        json(res, 200, await daemon.reading.words({ fresh: url.searchParams.get('fresh') === '1' }));
      } catch (err) {
        json(res, 200, { ok: false, error: err.message });
      }
    },
    ...
    'POST /api/reading/input': async (req, res) => {
      ...
      if (!daemon.reading.status().running) return json(res, 409, { ok: false, error: 'the reader is not open' });
      try {
        json(res, 200, await daemon.reading.input(body));
      } catch (err) {
        // An input kind the reader has no meaning for is the caller's mistake, and the only one of
        // these that is: everything else here is the reader failing at something it does mean.
        if (err instanceof UnknownInputError) return json(res, 400, { ok: false, error: err.message });
        json(res, 200, { ok: false, error: err.message });
      }
    },
    ...
    'POST /api/reading/session': async (req, res) => {
      json(res, 200, await daemon.reading.carrySession());
    },
    ...
    'POST /api/reading/signin': async (req, res) => {
      ...
      json(res, 200, { ok: true, ...daemon.reading.signIn({ asin: state?.book?.asin ?? null }) });
    },

    'GET /api/reading/signin': async (req, res) => {
      json(res, 200, { ok: true, ...daemon.reading.signInStatus() });
    },

    /**
     * Sign in in Safari, because that is where the keychain is.
     *
     * Returns as soon as Safari has been asked. There is nothing to wait for: unlike the
     * Chrome window, this one is not instrumented and cannot be. No panel control presses
     * this route yet; it is a manual path for an account whose passkey lives in iCloud
     * Keychain. See `Reader.startSafariSignIn`.
     */
    'POST /api/reading/signin/safari': async (req, res) => {
      ...
      try {
        json(res, 200, {
          ok: true,
          ...(await daemon.reading.signInSafari({ asin: state?.book?.asin ?? null })),
        });
      } catch (err) {
        json(res, 200, { ok: false, error: err.message });
      }
     ...
     * router's state snapshot: the pomodoro reading costs three screen captures a
    ...
    /**
     * Get the cards rung answering again. Anki is started behind everything and waited for, and
     * put back there if its launcher raised it anyway; the response's `hidden` says whether it went.
     */
    ...
    /** The undo for `{ day: true }`, which nothing else clears until the local day rolls over. */
    ...
    // Advances the ladder by one. It cannot select a rung: the router owns that choice, and a
    // debug route that overrode it would be testing a path the product does not have.
    'POST /debug/rung': async (req, res) => {
      if (!daemon.engine.status.open) return json(res, 409, { ok: false, error: 'no open gap' });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // One gate, before dispatch, rather than a check inside every handler in the table above:
    ...
    try {
      ...
      // A body this daemon could not parse is the caller's fault, not a fault in the handler that
      // never ran. 500 here would have sent an operator looking through the daemon log for a
  ...
  // The three shipped pages, held in memory and re-read only when the file on disk has moved.
  // A page load used to be an existsSync plus a full readFileSync inside a process that runs for
  // days, and the panel asks for its page again on every restart of the window.
  //
  // Gated on mtime rather than read once at startup, deliberately. `assetStamp` and the panel's
  // own reload are built on an edit to web/panel.html reaching a window that is already open, and
  // a cache with no expiry is just a slower way of not shipping the change. One stat per request
  // is the price of keeping that promise, and it is a fraction of the read it replaces.
  //
  // Per server rather than per module, because what is stored is the page with this daemon's
  // control token already injected. A module-level cache would hand a second server in the same
  // process the first one's token, which the tests do stand up.
  const pages = new Map();
  let assetReads = 0;
  ...
  function serveFile(res, file, type) {
    ...
    try {
      ...
      return json(res, 404, { error: `missing ${path.basename(file)}` });
    }
    const hit = pages.get(file);
    // Size as well as mtime: two edits inside the same clock tick are unlikely rather than
    // impossible, and a stale panel is exactly the bug this cache must not introduce.
    let body = hit && hit.mtimeMs === stat.mtimeMs && hit.size === stat.size ? hit.body : null;
    if (!body) {
      ...
      pages.set(file, { mtimeMs: stat.mtimeMs, size: stat.size, body });
    }
  ...
  return {
    server,
    port: config.port,
    stats: () => ({ panelServes, assetReads, asset: assetStamp() }),
    token: controlToken,
