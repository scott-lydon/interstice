...
export async function createServer({ daemon, config }) {
  ...
  const routes = {
    ...
    'POST /api/reading/view': async (req, res) => {
      // Started together. Draining the request body waits on the panel's socket and
      // the snapshot waits on the Kindle store, and neither is an input to the other,
      // so this route is called on every poll while the book is on screen.
