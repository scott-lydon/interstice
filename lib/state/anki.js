 ...
 *
 * Starting Anki is not a thing the router may do to you at 2am, so it happens only from the
 * cards rung: the button, or one automatic attempt a minute while that rung is the view you
 ...
 *
 * Ties are broken by the larger deck: at the same score, a deck with eleven cards
 * waiting was more neglected than one with a single card. Ties are common, because
 * a deck every card of which has been seen once scores exactly 1.0.
 */
...
/**
 * Your configured deck, or the least studied one when none is set, and its cards, ready to render.
 *
...
export async function leastStudiedQueue(config) {
  ...
  // The configured deck need not be the least studied one, so neither the reason nor the rank
  // can be a constant: the panel states both to the user, and README promises the arithmetic
  // can be checked rather than trusted.
  const chosenRank = ranking.indexOf(chosen) + 1;
  return {
    deck: chosen.deck,
    reason: chosenRank === 1 ? 'least_studied' : 'configured_deck',
