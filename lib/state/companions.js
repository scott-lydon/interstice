 ...
 *
 * Two captures of the same strip, a little over a second apart. A running countdown
 * has changed by the second capture; a paused one has not. This distinguishes paused
 * from running, which matters, because a paused pomodoro is the exact state you end
 * up in without noticing.
 */
/**
...
/**
 * A countdown ticks every second, so with samples spaced further apart than that,
 * every consecutive pair differs. All the same is a frozen display. One pair
...
/** What the panel says for each verdict, and whether it is worth interrupting for. */
...
export function invalidateCompanions() {
  cache = null;
}

export async function companionsSnapshot(config, { force = false, overrides = {} } = {}) {
  if (config.companions?.enabled === false) {
    return { enabled: false, at: Date.now(), companions: [] };
  }
  const ttl = config.companions?.cacheMs ?? 15000;
  if (!force && cache && Date.now() - cache.at < ttl) return applyOverrides(cache, overrides);
  ...
  cache = { enabled: true, at: Date.now(), companions: [binaural, pomodoro] };
  return applyOverrides(cache, overrides);
}
