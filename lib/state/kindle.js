...
export function recallBooks() {
  try {
    const c = JSON.parse(fs.readFileSync(READING_CACHE, 'utf8'));
    return c.book ? c : null;
  } catch {
    return null;
  }
}
...
/**
 * One read of the store at a time, and never one we cannot walk away from.
 *
 ...
 *
 * So a stuck read is abandoned rather than waited on, and no second one is started
...
export async function kindleState(config = {}, { home = os.homedir() } = {}) {
  ...
  try {
    ...
    const remembered = real ? recallBooks() : null;
