 ...
 * one note at a time is one Apple event per property per note: on this collection
 * (3,349 notes) that measured 104 seconds for the first 40 notes alone. Asking for
 * `a.notes.id()` gets every id in a single event. The whole collection then reads in
 * about 0.3s. Same data, one three-hundredth of the time.
 *
