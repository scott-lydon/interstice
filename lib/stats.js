 ...
 * Returns `{ enough: false, sample, needed }` when there is not enough history to say anything
 * honest, never null: a caller who writes `if (!t)` would never take that branch and would then
 * read `t.arm` as undefined.
 */
