...
export function isHumanSubmit(event) {
  if (!event || event.type !== 'user') return false;
  if (event.isSidechain) return false; // subagent traffic, not you
  if (event.isMeta) return false; // system-injected context
  if ('toolUseResult' in event) return false; // tool result wearing a user costume
...
export function isTurnEnd(event) {
  ...
  return TURN_END_STOP_REASONS.has(event.message?.stop_reason);
}
 ...
 */
export function isPermissionRequest(event) {
  if (!event) return false;
  if (event.type === 'user' && event.toolDenialKind) return true;
  return event.type === 'permission-request';
}
